...
function copyFolder(dir, number) {
  try {
    return new Promise((resolve, reject) => {
      exec('cp -r /Users/gaurav/Desktop/'+number+'/ /Users/gaurav/Desktop/6/'+number+'' , (err, stdout, stderr) => {
        if (err) {
          reject(err);
          return;
        }
...
return new Promise((resolve, reject) => {
  ...
  x.forEach(function(number) {
    getFilesFromDir("/Users/gaurav/Desktop/"+number.toString(), number).then(function(data) {
      resolve(data);
    })
  })
}).then(function(aaya) {
